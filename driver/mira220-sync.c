...
struct mira220 {
	...
	/* Trigger mode: 0 = master (free-running), 1 = slave (external trigger) */
	u32 trigger_mode;
};
...
static int mira220_write_start_streaming_regs(struct mira220 *mira220)
{
	...
	/*
	 * Set imager state based on trigger mode from device tree.
	 * trigger_mode = 0: master (free-running, generates trigger signals)
	 * trigger_mode = 1: slave (waits for external trigger from master)
	 */
	if (mira220->trigger_mode == 1) {
		imager_state = MIRA220_IMAGER_STATE_SLAVE_CONTROL;
		dev_info(&client->dev,
			 "Starting in SLAVE mode (external trigger)\n");
	} else {
		imager_state = MIRA220_IMAGER_STATE_MASTER_CONTROL;
		dev_info(&client->dev,
			 "Starting in MASTER mode (free-running)\n");
	}

	ret = cci_write(mira220->regmap, MIRA220_IMAGER_STATE_REG,
			imager_state, NULL);
	if (ret) {
		dev_err(&client->dev, "Error setting imager state control");
		return ret;
	}
...
static int mira220_probe(struct i2c_client *client)
{
	...
	/*
	 * Read trigger mode from device tree.
	 * ams,trigger-mode = <0> or absent: master (free-running, default)
	 * ams,trigger-mode = <1>: slave (external trigger via 0x1003 = 0x08)
	 */
	mira220->trigger_mode = 0;
	of_property_read_u32(dev->of_node, "ams,trigger-mode",
			     &mira220->trigger_mode);
	if (mira220->trigger_mode > 1) {
		dev_warn(dev,
			 "Invalid ams,trigger-mode=%u, defaulting to 0 (master)\n",
			 mira220->trigger_mode);
		mira220->trigger_mode = 0;
	}
	dev_info(dev, "MIRA220 trigger mode: %s\n",
		 mira220->trigger_mode ? "slave (external)" : "master (free-running)");
